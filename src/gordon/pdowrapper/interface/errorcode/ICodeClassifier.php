 ...
 *
 * This is an interface for classes that attempt to classify a given SQL result codes based on what response is reasonable
 * to make for them.  A given code could potentially be ignored, be indicative of a temporary error that may be recovered
 * from if the last query (or transaction) was replayed, or indicative of a temporary error that may be recovered from by
 * re-connecting to the database server and replaying the last query (or transaction)
 *
...
interface ICodeClassifier
{
     ...
     * replay the last query/transaction after recovery, though an additional step of reconnecting to the database server
     * has to be successfully accomplished first, and it may also be necessary to re-prepare any prepared statements that
     * were in use at the time the connection was lost.
     *
     * @param IErrorCodeEnum $code
     * @return bool
     */
